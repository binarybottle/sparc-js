...
// Set up the waveform chart
function setupWaveformChart() {
  ...
  waveformChart = new Chart(waveCtx, {
    type: 'line',
    data: {
      labels: Array(waveformHistory.length).fill(''),
      datasets: [{
        label: 'Waveform',
    ...
    options: {
      responsive: true,
      maintainAspectRatio: false, // This is important to control the size
      animation: { duration: 0 }, // For better performance
      plugins: {
...
function setupCharts() {
  ...
  // Add vocal tract outlines to the 2D chart
  //addVocalTractOutlines();
}
...
async function extractFeaturesLoop() {
  ...
  try {
    ...
    // The wavlmOutput is already the Layer 9 features since the model is truncated
    // You can now apply the linear projection to get articulatory features
...
******************************************************************************/
function addVocalTractOutlines() {
  // Only add outlines if they don't already exist
  if (xyPositionsChart.data.datasets.some(ds => ds.label === 'Vocal Tract Outline')) {
    return;
  }

  // Define the vocal tract outline points
  // These are simplified anatomical references in the same coordinate space as the articulators
  const palateOutline = [
    { x: 1.2, y: -1.5, label: "Upper Lip" },  // Upper lip starting point
    { x: 0.9, y: -1.7 },                       // Transition
    { x: 0.6, y: -1.5, label: "Alveolar Ridge" }, // Alveolar ridge area
    { x: 0.3, y: -1.2 },                       // Hard palate
    { x: 0.0, y: -1.0 },                       // Mid-palate
    { x: -0.3, y: -0.8 },                      // Transition to velum
    { x: -0.7, y: -0.6, label: "Velum" },      // Velum
    { x: -1.0, y: -0.4, label: "Pharynx" },    // Upper pharynx
    { x: -1.2, y: -0.2 }                       // Lower pharynx
  ];

  // The lower outline (jaw/tongue rest position)
  const jawOutline = [
    { x: 1.2, y: -0.5, label: "Lower Lip" },   // Lower lip
    { x: 0.9, y: -0.3 },                       // Front of floor of mouth
    { x: 0.6, y: -0.2, label: "Floor of Mouth" }, // Floor of mouth
    { x: 0.3, y: -0.2 },                       // Mid-floor
    { x: 0.0, y: -0.2 },                       // Back floor
    { x: -0.3, y: -0.2 },                      // Transition
    { x: -0.6, y: -0.2 },                      // Lower pharynx
    { x: -0.9, y: -0.2 }                       // Esophageal entrance
  ];

  // Add the palate outline to the chart
  xyPositionsChart.data.datasets.push({
    label: 'Vocal Tract Outline',
    data: palateOutline,
    showLine: true,
    fill: false,
    pointRadius: 0,
  ...
  // Add the jaw outline to the chart
  xyPositionsChart.data.datasets.push({
    label: 'Jaw Outline',
    data: jawOutline,
    showLine: true,
    ...
    borderDash: [],
    tension: 0.4,
    hidden: false // Show this by default
  });

  // Optional: Add reference points
  xyPositionsChart.data.datasets.push({
    label: 'Anatomical References',
    data: [
      { x: 1.2, y: -1.5, label: "Upper Lip" },
      { x: 1.2, y: -0.5, label: "Lower Lip" },
      { x: 0.6, y: -1.5, label: "Alveolar Ridge" },
      { x: -0.7, y: -0.6, label: "Velum" }
    ],
...
// Modify the waveform update logic
function updateWaveform(audioData) {
  // Update waveform data
  for (let i = 0; i < Math.min(50, audioData.length); i++) {
    const sampleIdx = Math.floor(i * (audioData.length / 50));
    waveformHistory.push(audioData[sampleIdx]);
    waveformHistory.shift();
  }
  
  // Calculate current loudness as indicator of phonation
  let sum = 0;
  for (let i = 0; i < audioData.length; i++) {
    sum += audioData[i] * audioData[i];
  }
  const rms = Math.sqrt(sum / audioData.length);
  const isPhonating = rms > 0.01; // Threshold for "is speaking"
  
  // Update waveform color based on phonation
  const color = isPhonating ? 
    'rgba(75, 192, 192, 0.8)' :  // Speaking: teal color
    'rgba(200, 200, 200, 0.6)';  // Silent: gray color
    
  waveformChart.data.datasets[0].borderColor = color;
  
  // Update the chart
  waveformChart.data.datasets[0].data = [...waveformHistory];
